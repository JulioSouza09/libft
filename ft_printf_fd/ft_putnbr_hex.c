...
/*   Updated: 2025/09/08 18:30:48 by jcesar-s         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_putnbr_hex(int fd, unsigned long nb, int letter_case)
{
	...
	if (nb > 15)
	{
		printed_chars += ft_putnbr_hex(fd, nb / 16, letter_case);
		nb %= 16;
	}
	if (letter_case == 'u')
		return (ft_putchar(fd, u_base[nb]) + printed_chars);
	else if (letter_case == 'l')
		return (ft_putchar(fd, l_base[nb]) + printed_chars);
	else
		return (-1);
}
